...
int ResetFrameNumbers::MainMethod()
{
	...
	int noOfCamera = FindNumberOfCameras(imageFolder);
	...
	if (isRenameFile) {
		...
		if (strcmp("Y", autoRenameFile) == 0) {
			...
			ResetFrameAuto(imageFolder, imageDestFolders);
		}
		else {
			...
			ResetFrameManual(imageFolder, imageDestFolders, frameMove);
		}
	}
	else {
		FindFrameCorresponding(imageFolder);
		//PrintTS(imageFolder);
	}
	return 1;
}
